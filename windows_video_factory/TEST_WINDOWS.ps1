# Windows CI verification entry point. Logs are captured by the workflow. Revision 4.
$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $MyInvocation.MyCommand.Path
Set-Location $root

New-Item -ItemType Directory -Force -Path images, output | Out-Null
$log = Join-Path $root 'output\\validation.log'
...
try {
  ...
  "PASS: MP4 generated ($duration seconds)" | Set-Content -Encoding UTF8 $log
